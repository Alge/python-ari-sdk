# coding: utf-8

"""
    Asterisk ARI

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 6.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from ari_async_sdk.api.bridges_api import BridgesApi  # noqa: E501


class TestBridgesApi(unittest.IsolatedAsyncioTestCase):
    """BridgesApi unit test stubs"""

    async def asyncSetUp(self) -> None:
        self.api = BridgesApi()

    async def asyncTearDown(self) -> None:
        await self.api.api_client.close()

    async def test_add_channel(self) -> None:
        """Test case for add_channel

        Add a channel to a bridge.  # noqa: E501
        """
        pass

    async def test_clear_video_source(self) -> None:
        """Test case for clear_video_source

        Removes any explicit video source in a multi-party mixing bridge. This operation has no effect on bridges with two or fewer participants. When no explicit video source is set, talk detection will be used to determine the active video stream.  # noqa: E501
        """
        pass

    async def test_create(self) -> None:
        """Test case for create

        Create a new bridge.  # noqa: E501
        """
        pass

    async def test_create_with_id(self) -> None:
        """Test case for create_with_id

        Create a new bridge or updates an existing one.  # noqa: E501
        """
        pass

    async def test_destroy(self) -> None:
        """Test case for destroy

        Shut down a bridge.  # noqa: E501
        """
        pass

    async def test_getbridge(self) -> None:
        """Test case for getbridge

        Get bridge details.  # noqa: E501
        """
        pass

    async def test_listbridges(self) -> None:
        """Test case for listbridges

        List all active bridges in Asterisk.  # noqa: E501
        """
        pass

    async def test_play(self) -> None:
        """Test case for play

        Start playback of media on a bridge.  # noqa: E501
        """
        pass

    async def test_play_with_id(self) -> None:
        """Test case for play_with_id

        Start playback of media on a bridge.  # noqa: E501
        """
        pass

    async def test_record(self) -> None:
        """Test case for record

        Start a recording.  # noqa: E501
        """
        pass

    async def test_remove_channel(self) -> None:
        """Test case for remove_channel

        Remove a channel from a bridge.  # noqa: E501
        """
        pass

    async def test_set_video_source(self) -> None:
        """Test case for set_video_source

        Set a channel as the video source in a multi-party mixing bridge. This operation has no effect on bridges with two or fewer participants.  # noqa: E501
        """
        pass

    async def test_start_moh(self) -> None:
        """Test case for start_moh

        Play music on hold to a bridge or change the MOH class that is playing.  # noqa: E501
        """
        pass

    async def test_stop_moh(self) -> None:
        """Test case for stop_moh

        Stop playing music on hold to a bridge.  # noqa: E501
        """
        pass


if __name__ == '__main__':
    unittest.main()
